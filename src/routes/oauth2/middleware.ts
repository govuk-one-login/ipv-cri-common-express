...
import { buildRedirectUrl } from "@src/lib/oauth";
...
const addAuthParamsToSession = async (
  req: Request,
  _res: Response,
  next: NextFunction,
) => {
  req.session.authParams = { client_id: req.query.client_id as string };
  next();
};

const addJWTToRequest = (req: Request, res: Response, next: NextFunction) => {
  req.jwt = req.query?.request as string;
  next();
};

const initSessionWithJWT = async (
  req: Request,
  _res: Response,
  next: NextFunction,
) => {
    ...
    if (requestJWT) {
      ...
      if (!apiResponse?.data["session_id"]) {
        logger
          ...
          .warn("No session ID in session response");
      }

      if (!apiResponse?.data?.redirect_uri) {
        logger
          .get(constants.PACKAGE_NAME)
          .warn("No redirect URI in session response");
      }
...
const retrieveAuthorizationCode = async (
  req: Request,
  _res: Response,
  next: NextFunction,
) => {
...
const redirectToCallback = async (
  ...
  try {
    ...
    logger.get(constants.PACKAGE_NAME).info("Redirecting to callback");
...
const redirectToEntryPoint = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
