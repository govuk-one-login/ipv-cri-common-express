...
const middleware =
  ...
  (err: AppError, req: Request, res: Response, _: NextFunction) => {
    ...
    if (res.finished || res.headersSent) {
      return logger
        .get()
        .error("Error after response: :clientip :verb :request :err.message", {
          req: req,
          err: err,
        });
    }
    ...
    if (!err.template) {
      ...
      logger.get().error(":clientip :verb :request :err.message", { req, err });
    }
